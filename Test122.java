...
○ 개요
	...
	이러한 클래스나 인터페이스의 묶음을 패키지(package)라고한다.
...
○ import구문
	...
	※ JDK 1.5 부터는 import다음에 『static』키워드를 붙임으로써
		정적으로 패키지를 import할 수 있는 기능을 제공하게 되었다.
		만약 패키지를 정적으로 import한 경우라면 모든 접근 가능한
		메소드와 멤버 변수들으 접두사(ex → ob, br, sc 처럼...)를 붙이지 않고
		사용하는 것이 가능하다.
...
※ Java Platform, Standard Edition & API Specification URL
...
public class Test122 //extends Object
{
	...
	*/
	public static void main(String[]args){
		...
		//Rect ob = Rect();
		//ob.input();
		//-- ob객체를 생성시키는 대상 클래스 → Rect클래스
		//	 Rect 클래스에 정의되어 있는 input()메소드 호출

		// 객체.xxx();
		//-- 해당 ㄷ객체를 생성시키는 대상 클래스에
		//	 xxx()메소드가 존재하지 않을 때...
		//	→ 에러발생
