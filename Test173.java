...
○ Serializable 인터페이스
	...
	java.io.Serializable 인터페ㅣ스를 구현해 주어야 한다.
	...
	ObjectInputStream 클래스와 ObjectOutputStream클래스를 이용하여
	객체 단위로 입추력을 수행하게 된다.
...
○ Object Stream
	...
	ObjectInputStram클래스는 ObjectOuptputStream 클래스에 의해
	파일에 저장되어 있는 객체나 네트워크를 통해 전달된 객체의
	직렬화를 해제하는 기능을 제공한다.
	단, java.io.Serializable 인터페이스를 지원해주는 객체에 대해서만
	사용이 가능하다.
...
public class Test173{
	public static void main(String[]args){
		...
		if (f0.exists())	//-- f0 파일 객체가 존재한다면
		{
			...
			// ois(ObjectInputStream)로 부터 읽어들인 객체(Object)를 (→ readObject를 통해 처리)
			//
